...
def NS_bubble(x, y, t, net):
    ...
    f_u = ((u_t + u * u_x + v * u_y) * rho + p_x - one_We * curvature * a_x - one_Re * (u_xx + u_yy) - 2.0 * one_Re_x * u_x - one_Re_y * (u_y + v_x)) / 100000
    ...
    f_v = ((v_t + u * v_x + v * v_y) * rho + p_y - one_We * curvature * a_y - one_Re * (v_xx + v_yy) - rho * one_Fr - 2.0 * one_Re_y * v_y - one_Re_x * (u_y + v_x)) / 100000
...
loss_path = '/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/loss.csv'
...
data_bc = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/data_bc.npy')
...
# IC
data_ic = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0-0.6/next_data_ic.npy')
...
# PDE
# data_pde = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/data_pde.npy')
# np.random.shuffle(data_pde)
data_pde = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/extended_data_pdes.npy')
# data_pde = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/extended_resample_data_pdes.npy')
...
for epoch in range(iterations):
    ...
    # torch.save(net, '/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/last_net.pth')
    if loss < min_loss:
        min_loss = loss
        # 保存模型语句
        torch.save(net, '/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/best_net_pde.pth')
        # torch.save(net, '/home/user/ZHOU-Wen/PINN/two-phase-flow/VOF/0.6-1.2/best_net_resample_pde.pth')
