...
def NS_bubble(x, y, t, net):
    ...
    mu = mu2 * (1 + a) / 2 + mu1 * (1 - a) / 2
    mu_x = mu2 / 2 * a_x - mu1 / 2 * a_x
    mu_y = mu2 / 2 * a_y - mu1 / 2 * a_y
    rho = rho2 * (1 + a) / 2 + rho1 * (1 - a) / 2

    # ψ=C(C^2-1)-ϵ^2 ∇^2 C
    phi = a * (a ** 2 - 1) - ε ** 2 * (a_xx + a_yy)
    ...
    # print(curvature)
    # curvature = - ((a_xx + a_yy) / abs_interface_grad - (a_x ** 2 * a_xx + a_y ** 2 * a_yy + 2 * a_x * a_y * a_xy) / torch.pow(abs_interface_grad, 3))
    # print(curvature)
    # print('xxxxxx')
    ...
    f_u = ((u_t + u * u_x + v * u_y) * rho + p_x - one_We * 3 * math.sqrt(2) / 4 * phi / ε * a_x - one_Re * (
    ...
    f_v = ((v_t + u * v_x + v * v_y) * rho + p_y - one_We * 3 * math.sqrt(2) / 4 * phi / ε * a_y - one_Re * (
                v_xx + v_yy) - rho * one_Fr - 2.0 * one_Re_y * v_y - one_Re_x * (u_y + v_x)) / 100000

    f_e = u_x + v_y

    f_a = a_t + u * a_x + v * a_y - M_0 * (phi_xx + phi_yy)
...
loss_path = '/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/loss.csv'
...
data_bc = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/data_bc.npy')
...
# IC
data_ic = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/data_ic.npy')
...
# PDE
# data_pde = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/data_pde.npy')
# np.random.shuffle(data_pde)
data_pde = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/extended_data_pdes.npy')
# data_pde = np.load('/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/extended_resample_data_pdes.npy')
...
for epoch in range(iterations):
    ...
    # torch.save(net, '/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/last_net.pth')
    if loss < min_loss:
        min_loss = loss
        # 保存模型语句
        torch.save(net, '/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/best_net_pde.pth')
        # torch.save(net, '/home/user/ZHOU-Wen/PINN/two-phase-flow/PF/0-0.6/best_net_resample_pde.pth')
